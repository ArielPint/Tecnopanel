...
import type { EstadoPagoIngreso } from '../types'

type EstadoPagoIngresoInput = Pick<
  EstadoPagoIngreso,
...
interface Props {
  ...
  onCrear: (input: EstadoPagoIngresoInput) => Promise<EstadoPagoIngreso>
  onActualizar: (id: string, patch: Partial<EstadoPagoIngresoInput>) => Promise<void>
}

// Guarda la lista de módulos/especialidades/partidas cubiertos por el EP —
// mismo patrón "reemplazar todo" que estados_pago/components/FormularioEstadoPago.tsx.
async function reemplazarModulos(estadoPagoIngresoId: string, descripciones: string[]) {
  const { error: delErr } = await supabase.from('estados_pago_ingresos_modulos').delete().eq('estado_pago_ingreso_id', estadoPagoIngresoId)
  if (delErr) throw new Error(delErr.message)
  if (descripciones.length === 0) return
  const { error: insErr } = await supabase
    .from('estados_pago_ingresos_modulos')
    .insert(descripciones.map((descripcion) => ({ estado_pago_ingreso_id: estadoPagoIngresoId, descripcion })))
  if (insErr) throw new Error(insErr.message)
}

export default function FormularioEstadoPagoIngreso({ estadoPago, trigger, onCrear, onActualizar }: Props) {
  ...
  useEffect(() => {
    ...
    supabase
      .from('estados_pago_ingresos_modulos')
      .select('descripcion')
      .eq('estado_pago_ingreso_id', estadoPago.id)
  ...
  const onSubmit = async (e: FormEvent) => {
    ...
    setEnviando(true)
    try {
      const input: EstadoPagoIngresoInput = {
