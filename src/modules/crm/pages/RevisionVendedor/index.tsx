...
export default function RevisionVendedor(){
  ...
  async function load(){
    ...
    const {data,error}=await supabase.from('oportunidades').select('*,cliente:clientes(razon_social,rut),vendedor:profiles(nombre,apellido)').in('id',idsPaso).in('etapa_actual',['Ventas','Ganado']).neq('tipo_venta','VIT').order('updated_at',{ascending:false})
