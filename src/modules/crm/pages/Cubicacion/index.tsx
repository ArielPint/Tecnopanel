...
export default function Cubicacion(){
  ...
  async function load(){
    const {data,error}=await supabase.from('oportunidades').select('*,cliente:clientes(razon_social,rut),vendedor:profiles(nombre,apellido)').eq('etapa_actual','Costos y Presupuestos').neq('tipo_venta','VIT').order('updated_at',{ascending:false})
    ...
    handleSupabaseError(asigsErr,'Cubicacion.load.asignaciones')
    handleSupabaseError(histErr,'Cubicacion.load.historial')
    ...
    if(userIds.length){
      ...
      handleSupabaseError(usersErr,'Cubicacion.load.profiles')
