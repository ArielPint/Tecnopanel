...
export default function Desarrollo(){
  ...
  async function load(){
    const {data,error}=await supabase.from('oportunidades').select('*,cliente:clientes(razon_social),vendedor:profiles(nombre,apellido)').eq('etapa_actual','Desarrollo').order('updated_at',{ascending:false})
    ...
    handleSupabaseError(asigsErr,'Desarrollo.load.asignaciones')
    handleSupabaseError(histErr,'Desarrollo.load.historial')
    ...
    if(userIds.length){
      ...
      handleSupabaseError(usersErr,'Desarrollo.load.profiles')
